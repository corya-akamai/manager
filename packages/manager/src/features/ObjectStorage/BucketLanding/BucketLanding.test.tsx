...
vi.mock('src/features/ObjectStorage/hooks/useObjectStorageRegions', () => ({
  useObjectStorageRegions: mocks.useObjectStorageRegions,
}));

vi.mock(
  'src/features/ObjectStorage/hooks/useObjectStorageRegionsWithAssignedEndpoints',
  () => ({
...
vi.mock('./BucketLandingFilters', () => ({
  BucketLandingFilters: (props: { onFiltersChange: (x: any) => void }) => (
    <button
      onClick={() =>
...
describe('BucketLanding', () => {
  /**
   * BucketLanding waits for the first onFiltersChange emit before rendering the buckets.
   * Our BucketLandingFilters mock exposes a single "filters" button that triggers that emit.
   */
  ...
  it('shows an error state when regions/endpoints fail to load', async () => {
    // BucketLanding renders the error UI when bucket fetching failed for all regions.
    ...
    // In this state the filters UI isn't rendered (BucketLanding returns early), so just assert on the error.
    expect(
